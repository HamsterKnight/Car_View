        ...
        <el-form class="content_form"
          ...
          <el-form-item prop="question">
            <el-input class="input"
              type="text"
              placeholder="输入密保问题"
              v-model="registerObj.question"></el-input>
          </el-form-item>
          <el-form-item prop="tempAnswer">
            <el-input class="input"
              type="text"
              placeholder="请输入密保答案"
              v-model="registerObj.tempAnswer"
          ...
          <el-form-item prop="confirmAnswer">
            <el-input class="input"
              type="text"
              placeholder="请输入确认答案"
              v-model="registerObj.confirmAnswer"
...
import { userRegister } from "@/http.js";
export default {
  name: "register",
  metaInfo: {
    // 设置 title
  ...
  methods: {
    registerAccount(formName) {
      ...
      this.registerObj.author = this.registerObj.account;
      this.$refs[formName].validate(valid => {
        if (valid && this.passwordFlag) {
          userRegister(this.registerObj).then(res => {
            if (res.data.code === 0) {
              this.$message({
                type: "success",
                message: "注册成功"
    ...
    changeTwo() {
      this.registerObj.answer = this.registerObj.tempAnswer;
      this.registerObj.tempAnswer = this.registerObj.tempAnswer.replace(
        /[\d\D]/g,
        "*"
      );
    },
    changeThree() {
      this.registerObj.confirm = this.registerObj.confirmAnswer;
      this.registerObj.confirmAnswer = this.registerObj.confirmAnswer.replace(
        /[\d\D]/g,
        "*"
      );
    },
...
.register_wrapper .content_form .vf_btn {
  ...
  background: url("../assets/images/hobby.jpg") no-repeat;
  font-size: 18px;
  font-weight: 500;
